    ...
    const handleDeleteEvent = async (eid) => {
        try {
            ...
            if (success) {
                ...
                onEventUpdated(); // Notify parent to re-fetch events
            } else {
                toast({
                    title: 'Error',
                    description: message,
    ...
    return (
        ...
        >
            ...
            <Box p={3}>
                ...
                <HStack spacing={2}>
                    { user.username ==='admin' && <IconButton icon={<EditIcon />} onClick={onOpen} colorScheme="blue" /> }
                    { user.username ==='admin' && <IconButton icon={<DeleteIcon />} onClick={() => handleDeleteEvent(event._id)} colorScheme="red" /> }
